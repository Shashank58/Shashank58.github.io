...
function change_colour(row, col, colour){
    ...
    changer.children[row].children[col].className = "grid-cell " + colour;
}
...
function flood(color){
    flushed[0][0] = true;
    MOVES++;
    var movesLeft = get_id("movesLeft");
...
function create_table(){
    ...
    for(var row=0; row<MAX_ROWS; row++){
        ...
        for(var col=0; col<MAX_COLS; col++){
            ...
            table_main.children[row].children[col].className = "grid-cell " + colour;
